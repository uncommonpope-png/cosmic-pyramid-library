...
function isAllowedConsoleError(entry) {
  ...
  if (/Failed to load resource/i.test(text) && /\.(?:glb|gltf|mp4|webm)(?:\?|$)/i.test(source)) return true;
  if (/net::ERR_(?:FAILED|ABORTED|BLOCKED_BY_CLIENT)/i.test(text) && /\.(?:glb|gltf|mp4|webm)(?:\?|$)/i.test(source)) return true;
  return false;
}

function isAllowedRequestFailure(entry) {
  return /(?:localhost|127\.0\.0\.1):(?:3001|3002|9001)\//i.test(entry.url) || /\.(?:glb|gltf|mp4|webm)(?:\?|$)/i.test(entry.url);
}
...
(async () => {
  ...
  try {
    ...
    await page.route('**/*', async (route) => {
      ...
      if (/\/assets\/.*\.(?:glb|mp4|webm)(?:\?|$)/i.test(url)) {
    ...
    const captureCanvas = async (fileName) => {
      ...
      const capture = await page.evaluate(() => {
        ...
        renderer.render(window.Genesis.scene, window.Genesis.camera);
      ...
      return { path: outputPath, png: await pngSignal(buffer), framebuffer: capture.framebuffer };
    };
    ...
    const checks = {
      ...
      heavyRoutesExplicitlyBlocked: blockedHeavyRoutes.length > 0,
      ...
      visibilityMetricsTruthful: baseline.genesis.visibility['heaven-prototype'].managedExternally === true && typeof baseline.genesis.visibility['heaven-prototype'].inRange === 'boolean' && typeof baseline.genesis.visibility['heaven-prototype'].inFrustum === 'boolean',
      ...
      surfaceRootsResume: returnedSurfaceRoots.length === baseline.stack.strata.surface.roots.length && returnedSurfaceRoots.every((rootEntry, index) => rootEntry.visible === baseline.stack.strata.surface.roots[index].visible),
    ...
    const result = {
      ...
      checks,
      canvasOnlySignals: Object.fromEntries(['surface', 'underside', 'prewarm', 'heaven', 'returned'].map((name, index) => [name, { png: framebufferSignals[index].png, framebuffer: framebufferSignals[index].framebuffer }])),
      screenshots,
      dependencies: { playwright: playwrightDependency.source, sharp: sharpDependency.source, browser: browserLaunch },
      blockedHeavyRoutes: { count: blockedHeavyRoutes.length, policy: 'Local GLB, MP4, and WebM routes are blocked to keep the focused probe deterministic.' },
      pageErrors,
      shaderErrors,
    ...
    if (failed.length) Object.assign(result, { failed, baseline, prewarm, heavenState, hiddenPortalAttempt, boundaryAttempt, beaconCollisionAttempt, returnedState });
